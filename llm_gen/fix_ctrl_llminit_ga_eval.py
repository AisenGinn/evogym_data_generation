...
import evogym.envs
from evogym import sample_robot, get_full_connectivity, hashable
...
def run_ga(
    ...
    ### MANAGE DIRECTORIES ###
    home_path = os.path.join("llminit_result", exp_name)
    results_path = os.path.join("llminit_result", exp_name, f"{env_name}_results.json")
    start_gen = 0
    ...
    if not is_continuing:
        temp_path = os.path.join("llminit_result", exp_name, "metadata.txt")
        
        try:
            os.makedirs(os.path.join("llminit_result", exp_name))
        except:
            pass
    ...
    #read status from file
        ...
        for g in range(start_gen+1):
            for i in range(pop_size):
                save_path_structure = os.path.join("llminit_result", exp_name, "generation_" + str(g), "structure", str(i) + ".npz")
    ...
    while True:
        ...
        save_path_structure = os.path.join("llminit_result", exp_name, "generation_" + str(generation), "structure")
        save_path_controller = os.path.join("llminit_result", exp_name, "generation_" + str(generation), "controller")
        ...
        temp_path = os.path.join("llminit_result", exp_name, "generation_" + str(generation), "output.txt")
        f = open(temp_path, "w")
